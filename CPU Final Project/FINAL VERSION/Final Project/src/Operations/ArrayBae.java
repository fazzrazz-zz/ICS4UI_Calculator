...
public class ArrayBae extends MainString {
    ...
    public static String useOperators(double firstAdjacent, double secondAdjacent, String symbol){
        ...
        if(symbol.equals("^")) return Operators.power(firstAdjacent, secondAdjacent);
        
        return "error";
    }
    
    public static String useOperators(double x, String symbol){
        
        if(symbol.equals("1/X")) return Operators.inverse(x);
        
        if(symbol.equals("√X")) return Operators.root(x);
        
        if(symbol.equals("sin")) return Operators.sine(x);
